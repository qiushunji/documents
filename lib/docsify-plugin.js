...
var myDocsifyPlugin = function(hook, vm) {
	...
	hook.afterEach(function(html) {
		...
		// 功能 2，文章底部添加仓库地址  
		var url = 'https://gitee.com/qiushunji/documents/blob/master/' + vm.route.file;
		var footer = [
			'<br/><br/><br/><hr/>',
			'<footer>',
			'<span>发现错误？ 您可以在 <a href="' + url + '" target="_blank">Gitee</a> 编辑此页文档！</span>',
			'</footer>'
		].join('');
		return html + footer;
	});
	...
	hook.doneEach(function() {
		...
		$('pre code').each(function() {
			if($(this).parent().parent().attr('class')=='docsify-tabs__content'){
				//如果是tabs则不要代码行了
				return;
			}
		...
		$('#main h2, #main h3, #main h4, #main h5, #main h6').each(function() {
			$('.toc-box').append('<li class="toc-' + this.localName + '">' + this.innerHTML +
				'</li>');
		});
	});
